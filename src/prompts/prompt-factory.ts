      ...
      messages: [
        {
          ...
          content: {
            ...
            text: [
              ...
              "",
              "To re-authorise, run one of these in the Xero MCP server directory — `npm run auth` if it is a git clone, or `npx xero-auth` if the package was installed from npm:",
              "",
              "    npm run auth",
              "",
              "If the server is configured with XERO_TOKEN_FILE, pass that same path to the command (`XERO_TOKEN_FILE=<path> npm run auth`) so it replaces the token file actually in use rather than creating one in the current directory.",
              "",
